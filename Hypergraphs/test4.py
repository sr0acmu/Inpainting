...
def load_image (image_file, config) :
    ...
    image = tf.image.resize (image, [int(config.image_shape[0]/4), int(config.image_shape[1]/4)])
    image = image / 255.0
...
def test (config) :
    if config.test_file_path != '' :
        ...
        for line in file.readlines () :
            ...
            input_image = np.zeros(( gt_image.shape[1]*4, gt_image.shape[2]*4, gt_image.shape[3]))
            shp = input_image.shape
            for i in range(0,shp[0]*4):
                for j in range(0,shp[2]*4):
                    if i%4==0 and j%4==0:
                        for k in range(0,shp[3]):
                            input_image[0][i][j][k] = gt_image[0][floor(i/4)][floor(j/4)][k]
                    else:
                        for k in range(0,shp[3]):
        ...
        for root, dirs, files in os.walk (config.test_dir) :
            for file in files :
                ...
                #input_image = np.where (mask==1, 1, gt_image)
                input_image = np.zeros((gt_image.shape[1]*4, gt_image.shape[2]*4, gt_image.shape[3]))
                for i in range(0,gt_image.shape[1]*4):
                    for j in range(0,gt_image.shape[2]*4):
                        if i%4==0 and j%4==0:
                            for k in range(0,gt_image.shape[3]):
                                input_image[i][j][k] = gt_image[0][floor(i/4)][floor(j/4)][k]
                        else:
                            for k in range(0,gt_image.shape[3]):
                                input_image[i][j][k] = 1.
                #print(input_image)
